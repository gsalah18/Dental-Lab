...
namespace Dental_Lab.Show_Forms
{
    public partial class ShowCustBills : Form
    {
        ...
        {
            if (tab_container.SelectedTab == payments_tab)
            {
                PaymentDataSet data = new PaymentDataSet();
                String payment_query = "SELECT pay_id as 'id', pay_price as 'price' " +
                    ", (SELECT COALESCE(SUM(ch_price), 0)" +
                    " FROM checks WHERE bill_id=payments.pay_id) as 'checks'" +
                    ", pay_date as 'date' , pay_note as 'note' " +
                    " FROM payments WHERE cust_id='" + Id + "'" +
                    " AND pay_date BETWEEN '" + FormatDate(datefrom_pic.Value) + "'" +
                    " AND '" + FormatDate(dateto_pic.Value) + "' ORDER BY pay_id DESC";
                ...
                if (MessageBox.Show("هل تريد الطباعه؟", "تاكيد الطباعه", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    string teethe_query = "SELECT " +
                        ...
                        "FROM teeth t WHERE t.bill_id=b.id";
                    string query = "SELECT b.id,bill_id,b.bill_teethes,b.bill_price" +
                        ", b.bill_delv_state" +
                        ",DATE_FORMAT(b.bill_delv_date,'%d %m  %Y') as 'bill_delv_date'" +
                        ", (" + teethe_query + ") as 'teeth'  " +
                        "FROM bill b WHERE b.cust_id='" + Id + "'" +
                        " AND b.bill_rec_date BETWEEN '" + FormatDate(datefrom_pic.Value) + "'" +
                        " AND '" + FormatDate(dateto_pic.Value) + "'";
